...
def augment_minority_class(frames, labels, noise_std=0.05):
    ...
    for frame in r_frames:
        ...
        noise3 = np.random.normal(0, 0.5 * noise_std * np.std(frame), size=frame.shape)
        augmented_frame3 = frame + noise3
        augmented_r_frames.append(augmented_frame3)
...
rf_model = RandomForestClassifier(
    n_estimators=5000,  # 增加决策树数量，提升稳定性
