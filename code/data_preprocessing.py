...
def get_match_key(file_name):
    ...
    try:
        ...
        if part_match:
            ...
            print(f" 未找到Part编号：{file_name}")
            return None

    except Exception as e:
        print(f" 文件名解析失败：{file_name} | 错误：{e}")
        return None
...
if matched_data:
    ...
    for idx, match_item in enumerate(matched_data, 1):
        ...
        try:
            ...
            if label_data.empty:
                print(f"【第{idx}组】{match_key} 标签文件为空，跳过")
                continue
            # 打印原始标签值，帮助匹配
            ...
            print(f"   有效标签总时长：{label_total_duration:.1f}秒 | 时长偏差：{abs(eeg_duration - label_total_duration):.1f}秒")
